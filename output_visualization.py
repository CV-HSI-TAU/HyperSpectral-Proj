...
def tensor_to_rgb(original, output, isPlot,device):
    RGB_hs=(HStorgb(original,device,0))
    RGB_NN=(HStorgb(output,device,1))
...
# def divide(x):
#     div =0
#     if (torch.amax(x[:, :, :, :] > abs(torch.amin(x[:, :, :, :]))
#         div = torch.amax(x[:, :, :, :]
#     else
#         div = abs(torch.amin(x[:, :, :, :])
#     return div
#for 10 WL
...
# for 150 WL
# def HStorgb(cube, device,flagg):
#    weights_r = torch.tensor([0.05, 0.9, 0.05]).to(device)
#    weights_g = torch.tensor([0.05,0.9,0.05]).to(device)
#    weights_b = torch.tensor([0.95,0.05]).to(device)
#    index_red = wave_lengths.index(700.8724832)
#    index_green = wave_lengths.index(550.8053691)
#    index_blue = wave_lengths.index(470)
#    cube = torch.permute(cube, (0, 2, 3, 1))
#    cube.to(device)
#    # print(torch.max(cube),torch.min(cube))
...
#    B = torch.matmul(cube[:, :, :, index_blue:index_blue+2].to(device), weights_b).unsqueeze(3)
#    R, G, B = map(lambda p: p / torch.amax(p[:, :, :, :], dim=(0, 1, 2)), [R, G, B])
#    R, G, B = map(lambda p: torch.clamp(p, min=0), [R, G, B])
#    if flagg==1:
#         R, G, B = map(lambda p: p*2, [R, G, B])
#    R = torch.permute(R, (0, 3, 1, 2))
